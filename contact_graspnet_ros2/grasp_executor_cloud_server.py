...
class GraspServer(Node):
    ...
    def __init__(self):
        ...
        self.caminfo_sub = self.create_subscription(
            CameraInfo, '/rgbd_camera/camera_info', self._caminfo_cb, 10
        )
    ...
    def _save_live_npy(self, points_flat, mask=None) -> str:
        ...
        # Save K if we have it
        if self.cam_K is not None:
            np.save(os.path.join(self.host_repo, 'test_data', 'K.npy'), self.cam_K)
    ...
    def run_inference_in_docker(self, np_relpath: str, *, use_local_regions: bool, use_filter_grasps: bool) -> dict:
        ...
        if use_filter_grasps:
         ...
         # Add K_path if present
        k_rel = 'test_data/K.npy'
        k_abs = os.path.join(self.host_repo, k_rel)
        if os.path.exists(k_abs):
            flags.append(f'--K_path={k_rel}')
