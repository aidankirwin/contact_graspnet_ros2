...
# --- Load RGB image ---
rgb_bgr = cv2.imread('./sample_scene_ucn/sample_2/input/from_rgbd-color.png')      # (480, 640, 3), BGR
rgb = cv2.cvtColor(rgb_bgr, cv2.COLOR_BGR2RGB)   # -> RGB uint8, like 0.npy['rgb']

# --- Load depth and convert to meters ---
depth_raw = cv2.imread('./sample_scene_ucn/sample_2/input/from_rgbd-depth.png', cv2.IMREAD_UNCHANGED).astype(np.float32)
...
seg = np.load('./sample_scene_ucn/sample_2/output/segmentation_from_rgbd/im_label.npy').astype(np.float32)  # (480, 640) float32; matches json["instance_ids"]
...
np.save('./sample_scene_ucn/sample_2/scene_from_ucn.npy', scene_dict)
